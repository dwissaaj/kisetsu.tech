...
export default function CardSearch(props: cardSearch) {
    ...
    return (
        <>
            <div key={props.id} className={'flex gap-2 p-4 border rounded-md border-teal-500 space-y-2 group  hover:bg-rose-500 transition duration-300 ease-in-out hover:outline-dotted hover:outline-2 hover:outline-offset-4 hover:outline-teal-700 hover:-translate-y-2'} >
                <Link className={'flex flex-col md:flex-row justify-center items-center gap-2'} href={`/blog/${slugUrl}`}>
                    ...
                    <div className={''}>
                        ...
                        <p className={'text-neutral-400 font-light truncate'}>{props.description}</p>
                    </div>
                </Link>
            </div>
        </>
