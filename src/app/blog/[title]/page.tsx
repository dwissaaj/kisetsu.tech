            ...
            <div className={'col-span-8 flex flex-col p-2 m-2 space-y-4 w-full items-center justify-center'}>
                <div className={'my-4 '}>
                    ...
                    <div className={'my-4 rounded-md space-y-3 flex flex-col justify-center items-center'}>
                        <Image className={'rounded-md shadow-md shadow-neutral-50'} width={700} height={700} src={`${data?.post?.image_header}`} alt={`${data?.post?.title}`}  priority={true} />
                        <p className={'text-light break-all'}>{data?.post?.description}</p>
                    </div>
                    <article className={'prose prose-sm lg:prose-lg break-words text-white my-4 w-full  prose-pre:bg-neutral-800'}>
